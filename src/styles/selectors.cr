module Caramel::Styles
  ...
  def regex_string_for_selector(selector : String) : String
  ...
  def regex_string_for_selector_group(group : String) : String
    items = group.split(">")
    re_items = items.map { |item| regex_string_for_selector_item(item) }
    re_items.join(RES_GT)
  end

  def regex_string_for_selector_item(item : String) : String
    t, k = item.split(".", 2) << ""
    if t == ""
      t = RES_TAG + RES_ID
    elsif t[0] == '#'
